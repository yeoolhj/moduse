...
export interface UseDefine<Define> {
    <InstanceThis extends ModuleRoot, T extends Define>(this: InstanceThis, defines: (T & ThisType<InstanceThis>) | T[], options?: {
...
export type UnionToIntersectionType<T> = (T extends any ? (x: {
    [name in keyof T]: T[name];
}) => any : never) extends (x: infer P) => any ? {
